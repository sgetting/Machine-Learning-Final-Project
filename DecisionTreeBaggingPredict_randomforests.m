...
predict_test = ones(Ntest,1); % Allocate space
...
%     predict_test(:,i) = score(:,2);
    predict_test = predict_test + (1/Nbag).*score(:,2);
end;
...
predict_train = ones(Ntest,1); % Allocate space
for i=1:Nbag, % Apply each classifier
    [label, score] = Classifiers{i}.predict(Xtrain(:,feat(i,:)));
%     predict_train(:,i) = score(:,2);
    predict_train = predict_train + (1/Nbag).*score(:,2);
end;
